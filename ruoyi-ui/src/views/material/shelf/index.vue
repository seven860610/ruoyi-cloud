    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['material:shelf:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['material:shelf:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['material:shelf:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listShelf, getShelf, delShelf, addShelf, updateShelf } from "@/api/material/shelf";
import { listInfo, getInfo, delInfo, addInfo, updateInfo } from "@/api/material/stock";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除【货架】编号为"' + ids + '"的数据项？').then(function() {
        return delShelf(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('material/shelf/export', {
        ...this.queryParams
    ...
    getTreeselect() {
      listInfo().then(response => {
        this.menuOptions = [];
        const menu = { id: 0, stockName: '请选择仓库', children: [] };
